 ...
 * 오프라인 데모 모드.
 *
 * Python 백엔드(FastAPI)가 없는 환경에서도 이 화면을 확인할 수 있도록,
 * index.html을 그냥 파일로 열거나(file://) 아무 정적 파일 서버로 띄웠을 때
 * (Node의 `npx serve`, VS Code Live Server 등 - Python이 전혀 필요 없음)
 * "/api/..." 요청을 가로채 브라우저 메모리 안의 가짜 데이터로 응답해준다.
 *
 * 동작 방식:
 ...
 *    모든 "/api/..." 요청을 아래 ROUTES로 처리한다. app.js는 fetch가
 *    가로채진 사실을 전혀 모른 채 평소와 똑같이 동작한다.
 *
...
(function () {
  ...
  const ids = { law: 0, revision: 0, document: 0, mapping: 0, candidate: 0, news: 0 };
  ...
  const ROUTES = [
...

    {
      ...
      handler: () => {
        ...
        return jsonResponse(200, {
          ...
          smtp_from: s.smtp_from || null,
          alert_emails: s.alert_emails || null,
    ...
    { method: "POST", re: /^\/api\/settings\/test-email$/, handler: () => errorResponse(400, "오프라인 데모 모드에서는 이메일을 보낼 수 없습니다.") },

...
- **안전보건 뉴스 탭(별도 페이지)**: 대시보드와 법령 마스터 사이에 있는 전용 탭(또는 게시판의
  "전체보기 · 검색" 버튼)에서 카테고리·제목 키워드·날짜 범위(시작일/종료일)로 뉴스를 검색합니다.
  검색은 서버(`GET /api/news/search`)에서 `published_at`(없으면 `fetched_at`) 기준으로 처리되며,
  검색 가능한 기간은 "카테고리별 최대 보관 건수"만큼 저장된 범위로 한정됩니다 - 더 오래된 기사까지
  찾으려면 그 값을 늘려두세요.

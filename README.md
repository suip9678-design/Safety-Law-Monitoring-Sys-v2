...
- `frontend/index.html`을 브라우저로 더블클릭해서 직접 열거나(`file://...`), Python 없이도 되는 아무
  정적 파일 서버(예: Node의 `npx serve frontend`, VS Code의 Live Server 확장)로 띄우면 됩니다.
- 페이지가 열리면 `frontend/demo-mode.js`가 실제 백엔드(`/api/health`)가 있는지 한 번 확인합니다.
  ...
  - 백엔드가 없으면 그때부터 모든 `/api/...` 요청을 가로채 브라우저 메모리 속 가짜 데이터(법령 5건,
    개정 이력 3건, 사규 2건, 뉴스 9건 등)로 응답해, 대시보드/법령 마스터/개정 이력/사규/안전보건 뉴스 등
    모든 탭을 실제로 눌러보고 등록·삭제·상태변경 같은 상호작용까지 확인할 수 있습니다.
  - 상단 배지가 "데모 모드 (오프라인)"로 바뀌어 있으면 이 모드로 동작 중이라는 뜻입니다. 데이터는 이
    브라우저 탭 메모리에만 있고 새로고침하면 초기 상태로 되돌아갑니다 - 확인용이지 실제 운영 데이터가
    아닙니다.
- 실제로 법령을 추적하고 이메일 알림을 받는 등 제대로 쓰려면 아래 "시작하기"대로 Python 백엔드를 설치해야
  합니다. 이 오프라인 데모는 그 전에 화면만 미리 보는 용도입니다.

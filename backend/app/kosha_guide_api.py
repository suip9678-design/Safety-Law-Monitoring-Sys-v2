...
차단됨) 아래 내용을 실제 응답으로 검증하지 못했다:
  - 요청 URL(BASE_URL): `https://apis.data.go.kr/B552468/srch` - 처음엔
    다른 KOSHA 서비스 패턴에서 추정한 값을 썼다가 틀렸었는데, 사용자가
    본인의 활용신청 상세페이지("일반 인증키"/"End Point" 안내 화면)를
    직접 캡처해 보내줘서 이 값으로 확정했다. 그래도 data.go.kr이 나중에
    경로를 바꾸면 설정 화면의 "요청 URL"에서 코드 수정 없이 바로 고칠 수 있다.

...
class KoshaGuide(Base):
    ...
    안전보건공단 홈페이지에 개별 PDF로 게시된다(이 개발 환경은 kosha.or.kr에
    직접 접근할 수 없어, 목록을 자동으로 받아올 방법이 없음을 실제로
    확인했다). 그래서 사용자가 한 건씩 입력하거나(가이드 추가) 갖고 있는
    목록을 붙여넣기(일괄 등록)로 채워두면, 그 범위 안에서 지침번호/제목/
    본문(입력해둔 경우)을 대상으로 로컬 키워드 검색을 제공한다 - 법령
    키워드 검색(ScrapedLawContent)과 같은 검색 경험을, 자동 수집이 안 되는
    이 데이터셋에 대해서도 제공하는 것이 목적이다."""

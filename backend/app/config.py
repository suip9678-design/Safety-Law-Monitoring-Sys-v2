...
class Settings:
    ...
    # 설명 참고. 이 URL은 사용자가 받은 공식 "오픈API 활용가이드" 문서의
    # "Call Back URL"/"요청메시지 예제"로 확정한 값이다(활용신청 상세페이지의
    # "End Point" 칸에는 이 중 서비스 URL 부분(.../srch)만 나오고 뒤의
    # 오퍼레이션 경로(/smartSearch)는 빠져 있어 처음에 놓쳤었다). 혹시 이후
